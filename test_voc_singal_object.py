...
# ----- output dir -----
out_dir = './seg_result_voc_singal_object'
...
for j in range(max_num_points):
    ...
    for name in tqdm(file_names, desc=f"points-{j+1}"):
        ...
        pred_bg = np.ones(label_np.shape[:2])
        for i in ids:
            if i == 0:
                continue
            ...
            cv.imwrite(os.path.join(result_dir, f"{name}_object{i}_point{j+1}_iou{iou_v}_f1score{f1_score_v}_time{spend_time}.png"), outputs*255)
